...
trait LiveCollectionPublisher extends LiveObjectReader with SyncMessagesGenerator with ObjectValidator with ObjectEnrichment with EcarGenerator with LiveObjectUpdater {

  private[this] val logger = LoggerFactory.getLogger(classOf[LiveCollectionPublisher])
  ...
  def getRelationalMetadata(identifier: String, readerConfig: ExtDataConfig)(implicit cassandraUtil: CassandraUtil): Option[Map[String, AnyRef]] = {
    val row: Row = getCollectionHierarchy(identifier, readerConfig)
  ...
  private def enrichChildren(toEnrichChildren: ListBuffer[Map[String, AnyRef]], collectionResourceChildNodes: mutable.HashSet[String], childNodesToRemove: ListBuffer[String])(implicit janusGraphUtil: JanusGraphUtil, cassandraUtil: CassandraUtil, readerConfig: ExtDataConfig, config: PublishConfig): ListBuffer[Map[String, AnyRef]] = {
    ...
    newChildren.map(child => {
      ...
      if (StringUtils.equalsIgnoreCase(child.getOrElse("visibility", "").asInstanceOf[String], "Default") && EXPANDABLE_OBJECTS.contains(child.getOrElse("objectType", "").asInstanceOf[String])) {
        ...
        val childCollectionHierarchy = getHierarchy(child.getOrElse("identifier", "").asInstanceOf[String], readerConfig).get
